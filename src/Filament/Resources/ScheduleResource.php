        ...
        return $form
            ->schema([
                Section::make([
                    ...
                    Forms\Components\Toggle::make('sendmail_search_words')
                        ->label(__('filament-database-schedule::schedule.fields.sendmail_search_words'))
                        ->default(false)
                        ->reactive(),
                    Forms\Components\Textarea::make('search_words')
                        ->label(__('filament-database-schedule::schedule.fields.search_words'))
                        ->helperText(__('filament-database-schedule::schedule.fields.search_words_helptext'))
                        ->reactive()
                        ->rows(function(Get $get): int {
                            $rows = count(explode("\n", $get("search_words")));
                            return $rows < 3 ? 3 : $rows; 
                        })
                        ->hidden(fn($get) => !$get('sendmail_search_words')),
